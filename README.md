...
DIYVM Local Passkey `1.2.7` 是一款面向通用网站的本地密码与通行密钥保险库。
...
- 可选“信任此设备”，不保存主密码，并在 Chrome 完全重启后自动恢复解锁；同一
  Chrome 用户配置的使用者也会因此获得保险库访问能力。
- 首次安装自动打开引导页，可一次点击启用通用 Passkey，也可跳过后在设置中开启。
...
- 主密码不保存；默认情况下，解锁后的 Vault Key 仅放在浏览器会话存储中，并受
  自动锁定限制。用户明确选择“信任此设备”后，扩展会另存一份由不可导出 AES-GCM
  设备密钥包装的 Vault Key，以便重启后自动解锁；该选择会降低本机访问安全性。
...
- 启用“信任此设备”后，任何能进入同一 Chrome 用户配置的人都能在无需主密码的情况
  下访问保险库；共享电脑上不应启用。
- 密码填入网页后，该网页自身的脚本可能读取输入框内容；只应在可信、域名正确的网站填充。
- 本项目完成了自动化测试和代码级安全检查，但不宣称已通过独立第三方安全审计。
- Chrome Web Store 中已经发布的旧版本与本仓库的 `1.2.7` 更新是不同审核批次；提交更新后
  仍需等待 Google 审核。
