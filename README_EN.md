<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/version-1.2.7-2458d3?style=flat-square" alt="Version 1.2.7">
...
DIYVM Local Passkey `1.2.7` is a local password and passkey vault for general websites.
...
- User-enabled WebAuthn passkey creation and sign-in on all HTTPS sites. When a sign-in page
  starts Conditional UI and matching local credentials exist, a DIYVM Passkey account list is
  shown below the login field with display labels, masked identifiers, and recent-use context.
  No website access is granted at installation.
...
- Optional trusted-device mode restores the unlocked vault after Chrome restarts without
  storing the master password; anyone using that Chrome profile can then access the vault.
...
- The master password is not stored. By default, the unlocked Vault Key lives only in
  `chrome.storage.session` and is subject to automatic locking. Trusted-device mode stores
  a Vault Key copy wrapped by a non-extractable AES-GCM device key for restart recovery,
  which deliberately lowers local profile-access security.
...
- Trusted-device mode lets anyone who can enter the same Chrome profile access the vault
  without the master password and should not be enabled on a shared computer.
...
- A previously approved Chrome Web Store version and the `1.2.7` update are separate review
