...
页面脚本拦截 `navigator.credentials.get()`，序列化 challenge、RP ID、允许凭据和用户
验证要求。后台只解密 RP ID 与允许列表同时匹配的本地凭据。

普通请求直接进入本地确认流程。对于 `mediation: "conditional"`：

1. 页面桥接立即启动原始 Chrome/系统 Conditional UI，避免扩展阻断网站登录；
2. 隔离世界向后台获取与当前 Origin、RP ID 和允许列表匹配的有界候选集；
3. 若保险库已解锁且有匹配项，在最可能的登录输入框下方显示封闭 Shadow DOM 中的
   DIYVM Passkey 账户列表。每行仅渲染别名/显示名称、脱敏账号和相对最近使用时间；
   原始凭据 ID 只保留在隔离世界内存中，不写入页面 DOM；
4. 用户点击账户后，后台再次验证该凭据属于当前请求，并在独立确认窗口中预选它；只有
   用户再次确认才进入签名流程。页面最多直接显示最近使用的 8 项，其余项在确认窗口选择；
5. 本地或原生验证器中最先成功的一方完成网页请求，并取消另一条本地等待流程。

本地匹配按 WebAuthn RP ID 进行，不要求登录页 URL 与创建凭据时的 URL 完全相同。
候选账户按最近使用时间排序。
...
页面取消、确认窗口关闭或本地确认流程 120 秒超时都会终止等待中的本地操作。
Conditional UI 在页面文档生命周期内由 Chrome 与本地候选并行等待；保险库未解锁、
没有匹配凭据、参数不受支持，或用户选择系统验证器时，本地候选退出，Chrome 原生流程
继续等待。用户明确取消整个请求时，两条流程都会终止。

不支持的算法/扩展参数、未获授权的网站和跨域 iframe 继续使用 Chrome 原生实现。
